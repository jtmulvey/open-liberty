 ...
 */
package com.ibm.ws.acme.internal.org.shredzone.acme4j.connector;

import static java.util.stream.Collectors.toList;
import static com.ibm.ws.acme.internal.org.shredzone.acme4j.toolbox.AcmeUtils.keyAlgorithm;
...
import com.ibm.ws.acme.internal.org.shredzone.acme4j.Problem;
...
import com.ibm.ws.acme.internal.org.shredzone.acme4j.exception.AcmeNetworkException;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.exception.AcmeProtocolException;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.exception.AcmeRateLimitedException;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.exception.AcmeRetryAfterException;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.exception.AcmeServerException;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.exception.AcmeUnauthorizedException;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.exception.AcmeUserActionRequiredException;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.toolbox.AcmeUtils;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.toolbox.JSON;
import com.ibm.ws.acme.internal.org.shredzone.acme4j.toolbox.JSONBuilder;
...
public class DefaultConnection implements Connection {
    ...
    public void sendRequest(URL url, Session session) throws AcmeException {
		...
		System.out.println("*** JTM *** DefaultConnection sendRequest URL path: "+url.toString());

        try {
        	System.out.println("DefaultConnection openConnection call with session.getProxy(): "+session.getProxy().toString());
            conn = httpConnector.openConnection(url, session.getProxy());
        	System.out.println("DefaultConnection openConnection call return");
            conn.setRequestMethod("GET");
            conn.setRequestProperty(ACCEPT_CHARSET_HEADER, DEFAULT_CHARSET);
            conn.setRequestProperty(ACCEPT_LANGUAGE_HEADER, session.getLocale().toLanguageTag());
            conn.setDoOutput(false);

            System.out.println("DefaultConnection call conn.connect() ...");

            conn.connect();

            System.out.println("DefaultConnection call conn.connect() returned ...");
            System.out.println("GET {} url " + url.toString() + " path: " + url.getPath());
            ...
            System.out.println("DefaultConnection caught IOException : "+ex);
            throw new AcmeNetworkException(ex);
        }
    }
    ...
    private int sendSignedRequest(URL url, JSONBuilder claims, Session session, KeyPair keypair, @Nullable URL accountLocation)
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                lastException = ex;
                System.out.println("Bad Replay Nonce, trying again (attempt {}/{}) " +attempt +"  max attempts " + MAX_ATTEMPTS);
            }
        }
    ...
    public JSON readJsonResponse() throws AcmeException {
        ...
        if (!("application/json".equals(contentType)
        	...
        	try {
        		System.out.println("*** JTM *** received response from LetsEncrypt: "+conn.getResponseMessage());
        	} catch (IOException ex) {
            	System.out.println("*** JTM *** I/O exception writing response.");
            }
        ...
        try {
            ...
            if (in != null) {
                ...
                System.out.println("Result JSON: {} " + result.toString());
            }
        } catch (IOException ex) {
            throw new AcmeNetworkException(ex);
        }
    ...
    public String getNonce() {
        ...
        System.out.println("Replay Nonce: {} " + nonceHeader);
    ...
    private int performRequest(URL url, JSONBuilder claims, Session session, KeyPair keypair,
        ...
        try {
            ...
            // if (LOG.isDebugEnabled()) {
                ...
                System.out.println("  JWS Header: {} " + jws.getHeaders().getFullHeaderAsJsonString());
            //}
        ...
        if (links != null) {
            ...
            for (String link : links) {
                ...
                if (m.matches()) {
                    ...
                    System.out.println("Link: {} -> {} " + relation +" Location "+ location);
                    result.add(location);
                }
            }
        }
